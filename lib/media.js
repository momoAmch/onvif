...
module.exports = function(Cam) {
	...
	*/
	 ...
	 */
	Cam.prototype.getMediaProfiles = Cam.prototype.getProfiles = function(callback) {
	 ...
	 * Receive stream URI
	 * @param {Object} [options]
	 * @param {string} [options.streamSetup]
	 * @param {string} [options.streamSetup.stream]
	 * @param {string} [options.streamSetup.transport]
	...
	Cam.prototype.getOSDs = function(token, callback) {
		...
		this._request({
			...
			if (!err) {
				...
				result = result.getOSDsResponse.OSDs;
			}
	 ...
	 * @param {string} [options.configurationToken] Token of the Video Source Configuration, which has associated OSDs
	...
	Cam.prototype.getOSDOptions = function(options, callback) {
		...
		this._request({
			...
			, body: this._envelopeHeader() +
				...
				'<ConfigurationToken>' + (options.videoSourceConfiguationToken ||
					options.configurationToken ||
					this.activeSource.videoSourceConfigurationToken) + '</ConfigurationToken>' +
				'</GetOSDOptions>' +
				this._envelopeFooter()
		}, function(err, data, xml) {
			if (callback) {
				callback.call(this, err, err ? null : linerase(data).getOSDOptionsResponse.OSDOptions, xml);
			}
		}.bind(this));
	};
